...
for i in listWalk:
    ...
    for x in range(len(statuses)):
        ...
        req.add_header('Content-Type', 'application/json')
        try:
            users = json.loads(urllib2.urlopen(req, json.dumps(
                {"function": "mailkit.mailinglist.getstatus", "id": "xxxx", "md5": "xxxx",
                 "parameters": {"status": status, "ID_user_list": listId, "ID_email": "0", "limit": pager}})).read())
        except Exception:
        ...
        if subscribersSum == pager:
            ...
            while True:
                req = urllib2.Request('https://api.mailkit.eu/json.fcgi')
                req.add_header('Content-Type', 'application/json')
                pagedUsers = json.loads(urllib2.urlopen(req, json.dumps(
                    {"function": "mailkit.mailinglist.getstatus", "id": "xxxx", "md5": "xxxx",
                     "parameters": {"status": status, "ID_user_list": listId, "ID_email": lastItem,
                                    "limit": pager}})).read())
        ...
        if listId == dmListId:
            ...
            for un in uniq:
                ...
                dmIntOutput.append(
                    "<td data-type=\"recipients dmRecipients\" data-state=\"" + status + "\" data-interest=\"" + un + "\">" + status + "/" + un + ":" + str(
                        dmUsersPerIssueAndState) + "</td>")
            dmIntOutput.append("</tr>")
