...
def reader(idson, dict):
    ...
    dict = {}
    bPager = 1
    basePath = "http://www.financnisprava.cz/cs/financni-sprava/drazby-financnich-uradu?page=" + str(
        bPager) + "&stav=" + str(idson) + "&druh=&urad=&from=&to=&minprice=&maxprice="
    bPager = 0
    auctionIndex = 0
    auctionIndexId = {}
    fileIndex = 0
    ...
    for mainIndex in htmlP.find_all('ul', {'class': 'archive-years pageList multiline'}):
        for li in mainIndex.findAll('li'):
            bPager = bPager + 1
            pagedPath = "http://www.financnisprava.cz/cs/financni-sprava/drazby-financnich-uradu?page=" + str(
                bPager) + "&stav=" + str(idson) + "&druh=&urad=&from=&to=&minprice=&maxprice="
            print(pagedPath)
            ...
            for a in pagedHtmlP.find_all("a", text=re.compile('(.)zemn(.) pracovi(.)t(.) (v|pro)(.*)')):
                detailLink = a['href']
                print(detailLink)
                for aucId in re.finditer(
                        'http:\/\/www\.financnisprava\.cz\/cs\/financni\-sprava\/drazby\-financnich\-uradu\/app\/detail\/(.*)',
                        detailLink):
                    aucId = aucId.group(1)
