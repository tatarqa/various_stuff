            ...
            for li in uls.findAll('li'):
                for a in li.findAll('a'):
                    ...
                    try:
                        ...
                        resultsSumNumberCont = detailParse.find('div',
                                                                {'class': 'filterTabHeadItems filterResultsText'})
                        if resultsSumNumberCont:
                            ...
                            if resultsSumNumberMatch:
                                ...
                                fin.write("[*] ID:%s " % (
                                detailId) + basePath + linka + "?byZone=marvel_site_zone&offset=0&byType=comic_series&dateStart=&dateEnd=&type=&orderBy=release_date+desc&byId=%s&limit=8&count=10&totalcount=%s" % (
                                          int(detailId[0]), int(resultsSumNumber[0])) + '\n')
                            else:
